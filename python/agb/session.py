...
class Session:
    ...
    def set_labels(self, labels: Dict[str, str]) -> OperationResult:
        ...
        try:
            ...
            log_operation_start(
                "Session.set_labels",
                f"SessionId={self.session_id}, LabelsCount={len(labels)}",
            )
            ...
            if response.is_successful():
                result_msg = (
                    f"SessionId={self.session_id}, RequestId={response.request_id}"
                )
                log_operation_success("Session.set_labels", result_msg)
                return OperationResult(
                    request_id=response.request_id or "", success=True
                )
            else:
    ...
    def get_labels(self) -> OperationResult:
        ...
        Raises:
            SessionError: If the operation fails.
        """
        log_operation_start("Session.get_labels",
        ...
        try:
            ...
            if response.is_successful():
                ...
                return OperationResult(
                    request_id=response.request_id or "", success=True, data=labels
                )
            else:
                # Get error message from response
    ...
    def info(self) -> OperationResult:
        ...
        """
        log_operation_start(
            "Session.info", f"SessionId={self.get_session_id()}")
        try:
            ...
            if hasattr(response, "is_successful"):
                ...
                if response.is_successful():
                    try:
                        ...
                        log_operation_error(
                            "Session.info",
                            f"Error parsing resource data: {str(e)}",
                            exc_info=True,
                        )
                        return OperationResult(
                            request_id=request_id,
    ...
    def delete(self, sync_context: bool = False) -> DeleteResult:
        ...
        """
        log_operation_start(
            "Session.delete", f"SessionId={self.session_id}, SyncContext={sync_context}"
        )
        try:
            import time
            import asyncio
            ...
            if not response.is_successful():
                ...
                logger.debug(
                    f"Full response: {json.dumps(response.to_map(), ensure_ascii=False, indent=2)}"
                )
                return DeleteResult(
                    request_id=request_id,
                    success=False,
            ...
            while True:
                ...
                if elapsed_time >= poll_timeout:
                    error_message = (
                        f"Timeout waiting for session deletion after {poll_timeout}s"
                    )
                    log_operation_error("Session.delete", error_message)
                    return DeleteResult(
                        request_id=request_id,
                    ...
                    is_not_found = (
                        ...
                        or (
                            ...
                            and (
                                ...
                                or "not found" in error_code.lower()
                            )
                        )
    ...
    def get_status(self) -> "SessionStatusResult":
        ...
        Returns:
            SessionStatusResult: Result containing session status information.
                ...
                - request_id (str): Unique identifier for this API request
        """
        log_operation_start("Session.get_status",
                            f"SessionId={self.session_id}")
        try:
    ...
    def keep_alive(self) -> OperationResult:
        ...
        This method calls the RefreshSessionIdleTime API to reset the idle timeout
        counter, preventing the session from being automatically released due to
        inactivity.
        ...
        Returns:
            OperationResult: Result indicating success or failure with request ID.
                ...
                - request_id (str): Unique identifier for this API request
        """
        log_operation_start("Session.keep_alive",
                            f"SessionId={self.session_id}")
        try:
            ...
            response = self.get_client().refresh_session_idle_time(request)
            ...
            if not response.is_successful():
                error_msg = response.get_error_message() or "Unknown error"
                log_operation_error("Session.keep_alive", error_msg)
                return OperationResult(
                    request_id=request_id,
                    success=False,
                    error_message=error_msg,
            ...
            log_operation_success(
                "Session.keep_alive",
                f"SessionId={self.session_id}, RequestId={request_id}",
            )
            return OperationResult(request_id=request_id, success=True)

        except Exception as e:
            log_operation_error("Session.keep_alive", str(e), exc_info=True)
            return OperationResult(
                request_id="",
                success=False,
                error_message=f"Failed to refresh session idle time: {e}",
            )
    ...
    def list_mcp_tools(self, image_id: Optional[str] = None) -> McpToolsResult:
        ...
        Args:
            image_id (Optional[str], optional): Image ID. Falls back to the
                session's image_id if not provided. Raises ValueError if
                neither is set.
        ...
        Raises:
            ValueError: If image_id is not provided and session has no image_id.
        ...
        Example:
            ...
            ```
        """
        image_id_str = image_id or getattr(self, "image_id", "")
        if not image_id_str:
            raise ValueError(
                "image_id is required. Provide it as a parameter or set it on the session."
            )

        log_operation_start(
            "Session.list_mcp_tools",
            f"SessionId={self.session_id}, ImageId={image_id_str}",
        )
        try:
            ...
            log_operation_error("Session.list_mcp_tools",
                                str(e), exc_info=True)
            return McpToolsResult(
                request_id="",
                success=False,
    ...
    def get_metrics(
        ...
        try:
            ...
            def _float_from_first_key(
                data: dict, keys: list, default: float = 0.0
            ) -> float:
            ...
            log_operation_error("Session.get_metrics",
                                error_msg, exc_info=True)
            return SessionMetricsResult(
                request_id=tool_result.request_id,
                success=False,
