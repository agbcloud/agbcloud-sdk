...
class Client:
    ...
    def refresh_session_idle_time(
        ...
        """Refresh session idle time using HTTP client."""
        if not request.authorization:
            raise ValueError("authorization is required")
        if not request.session_id:
            raise ValueError("session_id is required")
        http_client = self._get_http_client(request.authorization)
        try:
            return http_client.refresh_session_idle_time(request)
        finally:
            http_client.close()
    ...
    async def refresh_session_idle_time_async(
        ...
        """Refresh session idle time asynchronously using HTTP client."""
        if not request.authorization:
            raise ValueError("authorization is required")
        if not request.session_id:
            raise ValueError("session_id is required")
        http_client = self._get_http_client(request.authorization)
        try:
            return await http_client.refresh_session_idle_time_async(request)
        finally:
            http_client.close()
    ...
    def get_cdp_link(
        ...
        """
        Get CDP link using HTTP client.

        Args:
            request: Request object for getting CDP link.
        ...
        try:
            response = http_client.get_cdp_link(request)
            return response
        finally:
            http_client.close()
