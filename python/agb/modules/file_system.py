...
class FileSystem(BaseService):
        ...
        *,
        ...
        try:
            ...
            if result.success:
                ...
                log_operation_error(
                    "FileSystem.upload", result.error_message or "Upload failed"
                )
            return result
        ...
        *,
        ...
        try:
            ...
            if result.success:
                ...
                log_operation_error(
                    "FileSystem.download", result.error_message or "Download failed"
                )
            return result
        except Exception as e:
    ...
    def edit(
        ...
        """
        log_operation_start(
            "FileSystem.edit", f"Path={path}, EditsCount={len(edits)}, DryRun={dry_run}"
        )
    ...
    def move(self, source: str, destination: str) -> BoolResult:
        ...
        """
        log_operation_start(
            "FileSystem.move", f"Source={source}, Destination={destination}"
        )
    ...
    def _read_file_chunk(
        ...
        try:
            log_operation_start(
                "FileSystem.read",
                f"Path={path}, Offset={offset}, Length={length}, Format={format_type}",
            )
            ...
            if result.success:
                if format_type == "binary":
                    ...
                    try:
                        log_operation_success(
                            ...
                            f"result={str(result.data)}, RequestId={result.request_id}",
                        )
                        import base64
                        ...
                        log_operation_error(
                            "FileSystem.read.binary", str(e), exc_info=True
                        )
                        return BinaryFileContentResult(
                            request_id=result.request_id,
                            success=False,
                ...
                if format_type == "binary":
                    log_operation_error(
                        "FileSystem.read.binary",
                        result.error_message or "Failed to read file",
                    )
                    return BinaryFileContentResult(
                        request_id=result.request_id,
                        success=False,
    ...
    @overload
    def read(
        self, path: str, *, format: Literal["bytes"]
    ) -> BinaryFileContentResult: ...
        ...
        try:
            ...
            if format == "bytes":
                ...
                while offset < file_size:
                    ...
                    chunk_result = self._read_file_chunk(
                        path, offset, length, format_type="binary"
                    )
                ...
                while offset < file_size:
                    length = min(chunk_size, file_size - offset)
                    chunk_result = self._read_file_chunk(
                        path, offset, length, format_type="text"
                    )
                    if not chunk_result.success:
                        return chunk_result  # Return the error

                    if isinstance(chunk_result, FileContentResult):
    ...
    def write(self, path: str, content: str, mode: str = "overwrite") -> BoolResult:
        """
        ...
        log_operation_start(
            "FileSystem.write", f"Path={path}, Mode={mode}, ContentLength={content_len}"
        )
