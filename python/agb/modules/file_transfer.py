...
class FileTransfer:
    ...
    def ensure_context_id(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            log_operation_start(
                "FileTransfer.ensure_context_id",
                f"SessionId={self._session.get_session_id()}",
            )
                ...
                for item in data:
                    if isinstance(item, dict):
                        ...
                        if context_id and context_path:
                            ...
                            result_msg = (
                                f"ContextId={context_id}, ContextPath={context_path}"
                            )
                            log_operation_success(
                                "FileTransfer.ensure_context_id", result_msg
                            )
                            return True, ""
    ...
    def upload(
        ...
        *,
        ...
        log_operation_start(
            "FileTransfer.upload",
            f"LocalPath={local_path}, RemotePath={remote_path}, Wait={wait}",
        )
        ...
        if self.context_id is None:
            ...
            if not ensure_result:
                log_operation_error(
                    "FileTransfer.upload", f"Failed to ensure context_id: {message}"
                )
                return UploadResult(
                    success=False,
                    request_id_upload_url=None,
        ...
        url_res = self._context_svc.get_file_upload_url(self.context_id, remote_path)
        ...
        try:
            ...
            logger.info(
                f"Upload completed with HTTP {http_status}, BytesSent={bytes_sent}"
            )
            ...
            log_operation_error(
                "FileTransfer.upload", f"Upload exception: {str(e)}", exc_info=True
            )
            return UploadResult(
                success=False,
                request_id_upload_url=req_id_upload,
    ...
    def download(
        ...
        *,
        ...
        log_operation_start(
            ...
            f"RemotePath={remote_path}, LocalPath={local_path}, Wait={wait}, Overwrite={overwrite}",
        )
        ...
        if self.context_id is None:
            ...
            if not ensure_result:
                log_operation_error(
                    "FileTransfer.download", f"Failed to ensure context_id: {message}"
                )
        ...
        try:
            ...
            req_id_sync = self._await_sync("upload", remote_path, self.context_id or "")
        except Exception as e:
        ...
        url_res = self._context_svc.get_file_download_url(self.context_id, remote_path)
        if not getattr(url_res, "success", False) or not getattr(url_res, "url", None):
        ...
        try:
            ...
            logger.info(
                f"Download completed with HTTP {http_status}, BytesReceived={bytes_received}"
            )
            if http_status != 200:
            ...
            log_operation_error(
                "FileTransfer.download", f"Download exception: {str(e)}", exc_info=True
            )
            return DownloadResult(
                success=False,
                request_id_download_url=req_id_download,
    ...
    def _await_sync(
        ...
        async def _call_sync():
            ...
            try:
                ...
                try:
                    return await sync_fn(mode=mode, path=dir_path)
                except TypeError:
