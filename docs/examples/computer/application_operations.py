...
def main():
    ...
    try:
        ...
        if installed_apps_result.success:
            ...
            print(f"  ❌ Failed to get installed apps: {installed_apps_result.error_message}")
            return
        ...
        if start_result.success and len(start_result.data) > 0:
            ...
            for process in start_result.data:
                ...
                print(f"      Process: PID={process_info['pid']}, Name={process_info['pname']}")
        else:
        ...
        if visible_apps_result.success:
            ...
            for started_proc in started_processes:
                ...
                if found:
                    print(f"    ✅ {started_proc['app_name']} (PID: {started_proc['pid']}) is visible")
                else:
                    print(f"    ⚠️  {started_proc['app_name']} (PID: {started_proc['pid']}) not found in visible apps")
        else:
        ...
        for process in pids_to_stop:
            ...
            if stop_result.success:
                ...
                print(f"    ❌ Failed to stop {app_name} by PID: {stop_result.error_message}")
            ...
            if start_result.success and len(start_result.data) > 0:
                ...
                if test_pname:
                    ...
                    if cleanup_result.success:
                        ...
                        print(f"      ⚠️  Cleanup failed: {cleanup_result.error_message}")
                
                time.sleep(2)
            else:
                print(f"    ❌ Failed to start test application: {start_result.error_message}")
