...
class TestSessionListMcpTools(unittest.TestCase):
    ...
    def test_list_mcp_tools_no_image_id_raises(self):
        """Test listing tools without image_id raises ValueError"""
        delattr(self.session, "image_id")

        with self.assertRaises(ValueError) as ctx:
            self.session.list_mcp_tools()

        self.assertIn("image_id is required", str(ctx.exception))
