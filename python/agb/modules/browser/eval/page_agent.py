...
from agb.modules.browser.browser_agent import (
    ActOptions,
    ExtractOptions,
    ObserveOptions,
    ActResult,
...
class PageAgent:
                    ...
                    for file in os.listdir(f"{pwd}/page_tasks"):
                        if file.endswith(".py") and file != "__init__.py":
                            ...
                            content = content.replace(
                                "mcp_server.page_agent", "agb.browser.eval.page_agent"
                            )
    ...
    async def run_task(
        self, task: ModuleType, logger: logging.Logger, config: Dict[str, Any]
    ) -> Any:
        arguments = {
            "task": task,
            "logger": logger,
