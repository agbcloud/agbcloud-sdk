...
class Command(BaseService):
    ...
    def execute(
        ...
        if envs is not None:
            ...
            for key, value in envs.items():
                ...
                if not isinstance(value, str):
                    invalid_vars.append(
                        f"value for key '{key}' (type: {type(value).__name__})"
                    )
        ...
        try:
            ...
            log_operation_start(
                "Command.execute",
                f"Command={command}, TimeoutMs={timeout_ms}, Cwd={cwd}, Envs={bool(envs)}",
            )
            ...
            if result.success:
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to parse JSON response, using old format: {e}"
                    )
