...
def test_browser_proxy_configuration():
    ...
    try:
        ...
        managed_map = managed_proxy.to_map()
        print(f"✅ Managed proxy created successfully!")
        print(f"   Type: {managed_proxy.type}")
        print(f"   Strategy: {managed_proxy.strategy}")
        print(f"   User ID: {managed_proxy.user_id}")
        print(f"   Map: {managed_map}")
        assert managed_map.get("userId") == "user-123"
        assert managed_map.get("country") == "US"
        restored_managed = BrowserProxy.from_map(managed_map)
        assert restored_managed.type == "managed"
        assert restored_managed.user_id == "user-123"

        # Test from_map method
        print("\n5. Testing from_map method...")
        ...
        try:
            BrowserProxy(proxy_type="managed", strategy="polling")
            print("❌ Should have raised ValueError for managed proxy without user_id")
        except ValueError as e:
            print(f"✅ Correctly caught validation error: {e}")
...
def test_get_cdp_link(session):
    """Test getCdpLink API directly after browser initialization"""
    print("\n" + "=" * 60)
    print("Testing getCdpLink API")
    print("=" * 60)
    ...
    try:
        ...
        print(f"1. Calling getCdpLink API...")
        print(f"   Session ID: {session.get_session_id()}")

        response = session.get_client().get_cdp_link(request)
        ...
        if response.status_code == 404:
            print("⚠️  getCdpLink API not deployed (404), skipping")
            return True
        ...
        if response.is_successful():
            ...
            if url and len(url) > 0:
                print("✅ getCdpLink API returned valid URL!")
                return True
            else:
                print("❌ getCdpLink API returned empty URL")
                return False
        else:
            print(f"❌ getCdpLink API failed: {response.get_error_message()}")
            return False

    except Exception as e:
        print(f"❌ Error occurred during getCdpLink test: {e}")
        traceback.print_exc()
        return False
...
async def main():
    ...
    try:
        ...
        # Test 7: getCdpLink API
        print("\n" + "=" * 60)
        print("TEST 7: getCdpLink API")
        print("=" * 60)

        cdp_link_success = test_get_cdp_link(session)
        test_results["get_cdp_link"] = cdp_link_success
        if cdp_link_success:
            print("✅ getCdpLink API test passed!")
        else:
            print("❌ getCdpLink API test failed!")

        # Test 8: Async browser initialization
        print("\n" + "=" * 60)
        print("TEST 8: Async Browser Initialization")
        print("=" * 60)
        ...
        # Test 9: Browser agent
        print("\n" + "=" * 60)
        print("TEST 9: Browser Agent")
        print("=" * 60)
