...
async def reformat_agent_history(
    ...
    for step_num, history_item in enumerate(agent_history.history):
        ...
        if history_item.state:
            try:
                ...
                screenshot_data_b64 = (
                    history_item.state.get_screenshot()
                    if hasattr(history_item.state, "get_screenshot")
                    else None
                )
                # Fallback to screenshot_path if available
                if not screenshot_data_b64 and hasattr(
                    history_item.state, "screenshot_path"
                ):
                    ...
                    if screenshot_path_attr:
                        ...
                        with open(screenshot_path_attr, "rb") as f:
                            screenshot_data_b64 = base64.b64encode(f.read()).decode(
                                "utf-8"
                            )
                # Final fallback to screenshot attribute if it exists
                elif not screenshot_data_b64 and hasattr(
                ...
                if screenshot_data_b64:
                    screenshot_path = (
                        trajectory_with_highlights_dir / f"step_{step_num}.png"
                    )
                ...
                logger.warning(
                    f"Failed to save screenshot for step {step_num}: {type(e).__name__}: {e}"
                )
    ...
    if complete_history and len(complete_history) > 0:
        ...
        if isinstance(first_step, dict) and isinstance(last_step, dict):
            ...
            if isinstance(first_step_metadata, dict) and isinstance(
                last_step_metadata, dict
            ):
...
class TaskResult:
    ...
    def __init__(
        ...
        self.failed_stages: dict[Stage, StageError] = (
            {}
        )  # Store errors from failed stages
