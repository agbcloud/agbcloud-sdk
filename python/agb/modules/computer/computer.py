...
class Computer(BaseService):
    ...
    def click_mouse(self, x: int, y: int, button=None, **kwargs):
        ...
        log_operation_start(
            "Computer.click_mouse", f"X={x}, Y={y}, Button={button_str}"
        )
        try:
            ...
            if result.success:
                result_msg = (
                    f"X={x}, Y={y}, Button={button_str}, RequestId={result.request_id}"
                )
    ...
    def move_mouse(self, x: int, y: int, **kwargs):
        ...
        try:
            ...
            return BoolResult(
                request_id="", success=False, error_message=f"Failed to move mouse: {e}"
            )

    def drag_mouse(
        self, from_x: int, from_y: int, to_x: int, to_y: int, button=None, **kwargs
    ):
        ...
        button_str = (
            (button.value if isinstance(button, MouseButton) else button)
            if button
            else kwargs.get("button", "left")
        )
        ...
        log_operation_start(
            "Computer.drag_mouse",
            f"FromX={from_x}, FromY={from_y}, ToX={to_x}, ToY={to_y}, Button={button_str}",
        )
        try:
            args = {
                "from_x": from_x,
            ...
            return BoolResult(
                request_id="", success=False, error_message=f"Failed to drag mouse: {e}"
            )
    ...
    def scroll(self, x: int, y: int, direction=None, amount: int = 1, **kwargs):
        ...
        direction_str = (
            ...
            if direction
            else kwargs.get("direction", ScrollDirection.UP.value)
        )
        ...
        log_operation_start(
            "Computer.scroll",
            f"X={x}, Y={y}, Direction={direction_str}, Amount={amount}",
        )
        try:
            ...
            return BoolResult(
                request_id="", success=False, error_message=f"Failed to scroll: {e}"
            )
        ...
        try:
            ...
            if result.success:
                return OperationResult(
                    request_id=result.request_id, success=True, data=result.data
                )
            else:
                return OperationResult(
                    request_id=result.request_id,
                    ...
                    or "Failed to get cursor position",
                )
        except Exception as e:
            return OperationResult(
                request_id="",
    ...
    def input_text(self, text: str, **kwargs):
        ...
        try:
            ...
            return BoolResult(
                request_id="", success=False, error_message=f"Failed to input text: {e}"
            )
    ...
    def press_keys(self, keys, hold: bool = False, **kwargs):
        ...
        try:
            ...
            return BoolResult(
                request_id="", success=False, error_message=f"Failed to press keys: {e}"
            )
    ...
    def resize_window(self, window_id: int, width: int, height: int, **kwargs):
        ...
        log_operation_start(
            ...
            f"WindowId={window_id}, Width={width}, Height={height}",
        )
        try:
        ...
        try:
            ...
            if not result.success:
                return InstalledAppListResult(
                    ...
                    success=False,
                    error_message=result.error_message
                    or "Failed to get installed apps",
                )
            try:
                apps_json = json.loads(result.data)
                installed_apps = [
                    InstalledApp._from_dict(app_data) for app_data in apps_json
                ]
                return InstalledAppListResult(
                    request_id=result.request_id, success=True, data=installed_apps
                )
            except json.JSONDecodeError as e:
                return InstalledAppListResult(
                    request_id=result.request_id,
            ...
            return InstalledAppListResult(
                success=False, error_message=f"Failed to get installed apps: {e}"
            )
    ...
    def list_visible_apps(self, **kwargs):
        ...
        try:
            ...
            try:
                ...
                processes = [
                    Process._from_dict(process_data) for process_data in processes_json
                ]
                return ProcessListResult(
            ...
            return ProcessListResult(
                success=False, error_message=f"Failed to list visible apps: {e}"
            )

    def start_app(
        self, start_cmd: str, work_directory: str = "", activity: str = "", **kwargs
    ):
        ...
        try:
            ...
            try:
                ...
                log_operation_success(
                    "Computer.start_app",
                    f"StartCmd={start_cmd}, ProcessesCount={len(processes)}, RequestId={result.request_id}",
                )
                return ProcessListResult(
                    request_id=result.request_id, success=True, data=processes
                )
            ...
            return ProcessListResult(
                success=False, error_message=f"Failed to start app: {e}"
            )
    ...
    def stop_app_by_pname(self, pname: str, **kwargs):
        ...
        try:
            ...
            if result.success:
                log_operation_success(
                    "Computer.stop_app_by_pname",
                    f"PName={pname}, RequestId={result.request_id}",
                )
            else:
                log_operation_error(
                    "Computer.stop_app_by_pname",
                    result.error_message or "Failed to stop app by pname",
                )
            return AppOperationResult(
                request_id=result.request_id,
                success=result.success,
                error_message=result.error_message
                or ("Failed to stop app by pname" if not result.success else ""),
            )
        except Exception as e:
            return AppOperationResult(
                success=False, error_message=f"Failed to stop app by pname: {e}"
            )
    ...
    def stop_app_by_pid(self, pid: int, **kwargs):
        ...
        try:
            ...
            if result.success:
                log_operation_success(
                    "Computer.stop_app_by_pid",
                    f"PID={pid}, RequestId={result.request_id}",
                )
            else:
                log_operation_error(
                    "Computer.stop_app_by_pid",
                    result.error_message or "Failed to stop app by pid",
                )
            return AppOperationResult(
                request_id=result.request_id,
                success=result.success,
                error_message=result.error_message
                or ("Failed to stop app by pid" if not result.success else ""),
            )
        except Exception as e:
            return AppOperationResult(
                success=False, error_message=f"Failed to stop app by pid: {e}"
            )
    ...
    def stop_app_by_cmd(self, stop_cmd: str, **kwargs):
        ...
        try:
            ...
            if result.success:
                log_operation_success(
                    "Computer.stop_app_by_cmd",
                    f"StopCmd={stop_cmd}, RequestId={result.request_id}",
                )
            else:
                log_operation_error(
                    "Computer.stop_app_by_cmd",
                    result.error_message or "Failed to stop app by cmd",
                )
            return AppOperationResult(
                request_id=result.request_id,
                success=result.success,
                error_message=result.error_message
                or ("Failed to stop app by cmd" if not result.success else ""),
            )
        except Exception as e:
            return AppOperationResult(
                success=False, error_message=f"Failed to stop app by cmd: {e}"
            )
    ...
    def screenshot(self, **kwargs):
        ...
        try:
            ...
            if result.success:
                log_operation_success(
                    "Computer.screenshot",
                    f"RequestId={result.request_id}, ImageUrl={result.data if result.data else 'None'}",
                )
                return OperationResult(
                    request_id=result.request_id, success=True, data=result.data
                )
