...
class Code(BaseService):
    ...
    def _parse_run_code_result(
        ...
        try:
            ...
            format_handlers = [
                (
                    "result",
                    lambda d: "result" in d and isinstance(d["result"], list),
                    self._parse_new_format,
                ),
                (
                    "rich",
                    lambda d: "logs" in d or "results" in d,
                    self._parse_rich_format,
                ),
    ...
    def _parse_result_item(self, res_item: Any) -> Optional[ExecutionResult]:
        ...
        if isinstance(res_item, dict):
            return ExecutionResult(
                ...
                chart=(
                    res_item.get("application/vnd.vegalite.v4+json")
                    or res_item.get("application/vnd.vegalite.v5+json")
                    or res_item.get("chart")
                ),
                is_main_result=(
                    res_item.get("isMainResult", False)
                    or res_item.get("is_main_result", False)
                ),
            )
    ...
    def _parse_rich_format(
        ...
        logs = ExecutionLogs(
            stdout=logs_data.get("stdout", []), stderr=logs_data.get("stderr", [])
        )
        ...
        if error_data:
            error_message = error_data.get("value", "") or error_data.get(
                "name", "UnknownError"
            )
