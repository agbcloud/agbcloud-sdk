...
class ContextService:
    ...
    def get(
        ...
        try:
            ...
            context_id = (
                context_id.strip() if context_id and context_id.strip() else None
            )
            ...
            if not context_id and not name:
                ...
                return ContextResult(
                    success=False, error_message=error_msg, request_id=""
                )
    ...
    def create(self, name: str) -> ContextResult:
        ...
        if not name or (isinstance(name, str) and not name.strip()):
            ...
            return ContextResult(success=False, error_message=error_msg, request_id="")
    ...
    def update(self, context: Context) -> OperationResult:
        ...
        if not context.id or (isinstance(context.id, str) and not context.id.strip()):
            ...
            return OperationResult(
                request_id="", success=False, error_message=error_msg
            )

        # Validate context.name
        if not context.name or (
            isinstance(context.name, str) and not context.name.strip()
        ):
        ...
        try:
            context_id = (
                context.id.strip() if isinstance(context.id, str) else context.id
            )
            context_name = (
                context.name.strip() if isinstance(context.name, str) else context.name
            )
            log_operation_start(
                "ContextService.update", f"ContextId={context_id}, Name={context_name}"
            )
            ...
            if not response.is_successful():
                error_msg = response.get_error_message()
                log_operation_error(
                    "ContextService.update", error_msg or "Unknown error"
                )
                return OperationResult(
                    request_id=request_id or "", success=False, error_message=error_msg
                )
    ...
    def delete(self, context: Context) -> OperationResult:
        ...
        if context is None:
            ...
            return OperationResult(
                success=False, error_message="context cannot be None", request_id=""
            )
        # Validate context.id
        if not context.id or (isinstance(context.id, str) and not context.id.strip()):
            error_msg = "context.id cannot be empty or None"
        ...
        try:
            ...
            if not response.is_successful():
                error_msg = response.get_error_message()
                log_operation_error(
                    "ContextService.delete", error_msg or "Unknown error"
                )
                return OperationResult(
                    request_id=request_id or "", success=False, error_message=error_msg
                )
    ...
    def get_file_download_url(self, context_id: str, file_path: str) -> FileUrlResult:
        ...
        if not context_id or (isinstance(context_id, str) and not context_id.strip()):
            ...
            return FileUrlResult(
                request_id="", success=False, url="", error_message=error_msg
            )
        ...
        validated_context_id = (
            context_id.strip() if isinstance(context_id, str) else context_id
        )
        validated_file_path = (
            file_path.strip() if isinstance(file_path, str) else file_path
        )
        log_operation_start(
            "ContextService.get_file_download_url",
            f"ContextId={validated_context_id}, FilePath={validated_file_path}",
        )
        ...
        if resp.is_successful():
            ...
            log_operation_error(
                "ContextService.get_file_download_url", error_msg or "Unknown error"
            )
    ...
    def get_file_upload_url(self, context_id: str, file_path: str) -> FileUrlResult:
        ...
        log_operation_start(
            "ContextService.get_file_upload_url",
            f"ContextId={validated_context_id}, FilePath={validated_file_path}",
        )
        ...
        if resp.is_successful():
            ...
            error_msg = resp.get_error_message()
            log_operation_error(
                "ContextService.get_file_upload_url", error_msg or "Unknown error"
            )
    ...
    def delete_file(self, context_id: str, file_path: str) -> OperationResult:
        ...
        if resp.is_successful():
            ...
            error_msg = resp.get_error_message()
            log_operation_error(
                "ContextService.delete_file", error_msg or "Unknown error"
            )
    ...
    def list_files(
        ...
        if not context_id or (isinstance(context_id, str) and not context_id.strip()):
            ...
            return ContextFileListResult(
                request_id="", success=False, entries=[], error_message=error_msg
            )
        ...
        validated_parent_folder_path = (
            parent_folder_path.strip()
            if parent_folder_path and isinstance(parent_folder_path, str)
            else (parent_folder_path or "")
        )
        ...
        if not resp.is_successful():
            error_msg = resp.get_error_message()
            log_operation_error(
                "ContextService.list_files", error_msg or "Unknown error"
            )
            return ContextFileListResult(
                request_id=request_id or "",
                success=False,
    ...
    def clear_async(self, context_id: str) -> ClearContextResult:
        ...
        try:
            ...
            result_msg = (
                f"ContextId={context_id}, Status=clearing, RequestId={request_id}"
            )
    ...
    def get_clear_status(self, context_id: str) -> ClearContextResult:
        ...
        try:
            log_operation_start(
                "ContextService.get_clear_status", f"ContextId={context_id}"
            )
            request = GetContextRequest(
                authorization=f"Bearer {self.agb.api_key}",
                id=context_id,
            ...
            result_msg = (
                f"ContextId={context_id}, Status={state}, RequestId={request_id}"
            )
            ...
            log_operation_error(
                "ContextService.get_clear_status", str(e), exc_info=True
            )
            return ClearContextResult(
                request_id="",
                success=False,
    ...
    def clear(
        ...
        log_operation_start(
            ...
            f"ContextId={context_id}, Timeout={timeout}s, PollInterval={poll_interval}s",
        )
        ...
        while attempt < max_attempts:
            ...
            if status == "available":
                ...
                result_msg = (
                    f"ContextId={context_id}, Status={status}, Elapsed={elapsed:.2f}s"
                )
