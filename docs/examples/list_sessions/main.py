...
# Add the project root to Python path so we can import agb
...
def main():
    ...
    if not api_key:
        ...
        print("Please set your API key: export AGB_API_KEY='your-api-key'")
        sys.exit(1)
    ...
    try:
        ...
        # Get first page
        result_page1 = agb.list(labels={"project": "list-demo"}, page=1, limit=2)
        ...
        while True:
            result = agb.list(labels={"owner": "demo-user"}, page=page, limit=limit)

            if not result.success:
                print(f"❌ Error on page {page}: {result.error_message}")
                break

            print(f"📄 Page {page}: Found {len(result.session_ids)} session IDs , Max results : {result.max_results}")
