"""
Moltbot on AGB — Slack Plugin Configuration Example

This script demonstrates an end-to-end workflow for provisioning a cloud session
with Moltbot preinstalled, configuring the Slack channel (Bot Token + App Token
for Socket mode) and optional model overrides, then opening the web console and
cleaning up on exit.
...
3. Enable and configure Slack when SLACK_BOT_TOKEN and SLACK_APP_TOKEN are set.
...
Create an app at https://api.slack.com/apps, enable Socket Mode, and create the
tokens. See: https://docs.openclaw.ai/channels/slack
...
Optional (channels and models):
  ...
  SLACK_BOT_TOKEN         Slack Bot Token (xoxb-...); required with SLACK_APP_TOKEN
                          to enable and configure the Slack plugin.
...
-----
  ...
  python openclaw_slack_config_example.py
"""
...
# -----------------------------------------------------------------------------
# User interaction: wait for Ctrl+C before releasing session
# -----------------------------------------------------------------------------
...
def main() -> None:
    ...
    slack_bot_token = _get_optional_env("SLACK_BOT_TOKEN")
    slack_app_token = _get_optional_env("SLACK_APP_TOKEN")
    ...
    try:
        ...
        # --- Step 3: Enable Slack plugin and set channel config (when env vars are set) ---
        ...
        if slack_bot_token is not None and slack_app_token is not None:
            enable_parts.append("moltbot plugins enable slack")
            config_set_parts.append(
                f'moltbot config set channels.slack.botToken "{slack_bot_token}"'
            )
            config_set_parts.append(
                f'moltbot config set channels.slack.appToken "{slack_app_token}"'
            )
            config_set_parts.append(
                'moltbot config set channels.slack.groupPolicy "open"'
            )
        ...
        if config_parts_list:
            ...
            config_command = " && ".join(config_parts_list)
            execute_command(session, config_command, timeout_ms=COMMAND_TIMEOUT_MS)
            print("\nRestarting Moltbot gateway...")
            execute_command(session, "moltbot gateway restart", timeout_ms=COMMAND_TIMEOUT_MS)
        else:
            print(
                "\nNo Slack credentials set (SLACK_BOT_TOKEN and SLACK_APP_TOKEN). "
                "Skipping Moltbot Slack plugin configuration."
            )
