...
class TestSessionPauseResumeIntegration(unittest.TestCase):
    ...
    def test_pause_and_resume_session_success(self):
        ...
        pause_result = self.agb.pause(session)
        ...
        resume_result = self.agb.resume(session, timeout=120, poll_interval=3)
    ...
    def test_pause_and_delete_session_success(self):
        ...
        self.assertTrue(get_result.success, f"Failed to get session: {get_result.error_message}")
        if hasattr(get_result.data, 'status') and get_result.data.status:
            self.assertIn(get_result.data.status, ["RUNNING", "STARTING", "CREATING"])
        else:
            print(f"  ✓ Session exists: {session.session_id}")
    ...
    def test_pause_nonexistent_session(self):
        ...
        pause_result = self.agb.pause(invalid_session)
    ...
    def test_resume_nonexistent_session(self):
        ...
        resume_result = self.agb.resume(invalid_session)
    ...
    def test_resume_async_nonexistent_session(self):
        ...
        # This should return a failed SessionResumeResult
        import asyncio
        resume_result = asyncio.run(self.agb.resume_async(invalid_session))
    ...
    def test_pause_already_paused_session(self):
        ...
        pause_result1 = self.agb.pause(session)
        ...
        pause_result2 = self.agb.pause(session)
        # Pausing an already paused session may have specific behavior
        # Just verify it doesn't crash and returns a proper result object
        self.assertIsInstance(pause_result2, SessionPauseResult)
    ...
    def test_resume_already_running_session(self):
        ...
        resume_result = self.agb.resume(session, timeout=30, poll_interval=2)
    ...
    def test_pause_resume_with_custom_parameters(self):
        ...
        # Pause with custom parameters (using agb method)
        print(f"\nStep 1: Pausing session with custom parameters...")
        pause_result = self.agb.pause(session, timeout=300, poll_interval=3.0)
        ...
        resume_result = self.agb.resume(session, timeout=300, poll_interval=3.0)
        ...
        get_result = self.agb.get_session(session.session_id)
        self.assertTrue(get_result.success)
        print(f"  ✓ Session status after resume with custom parameters: {get_result.data.status if hasattr(get_result.data, 'status') and get_result.data.status else 'Status not available'}")
...
class TestSessionPauseResumeEdgeCases(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not api_key:
            ...
            env_files = ['.env', '.history/.env_20251211110614', '.history/.env_20251211105830', '.history/.env_20251211105704']
        ...
        endpoint = get_test_endpoint()
        if not endpoint:
            # Try to get endpoint from .env file too
            for env_file in env_files:
                if os.path.exists(env_file):
                    load_dotenv(env_file)
                    endpoint = get_test_endpoint()
    ...
    def test_pause_with_short_timeout(self):
        ...
        # Pause with short timeout (using agb method)
        print(f"\nPausing session with short timeout...")
        pause_result = self.agb.pause(session, timeout=3, poll_interval=1)
