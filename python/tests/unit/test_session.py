...
class TestSession(unittest.TestCase):
    ...
    @patch("agb.session.RefreshSessionIdleTimeRequest")
    def test_keep_alive_success(self, MockRefreshSessionIdleTimeRequest):
        """Test successful keep_alive operation"""
        from agb.model.response import OperationResult

        mock_request = MagicMock()
        mock_response = MagicMock()
        MockRefreshSessionIdleTimeRequest.return_value = mock_request
        self.agb.client.refresh_session_idle_time.return_value = mock_response

        # Mock the response methods
        mock_response.is_successful.return_value = True
        mock_response.request_id = "request-keep-alive-123"

        result = self.session.keep_alive()

        self.assertIsInstance(result, OperationResult)
        self.assertEqual(result.request_id, "request-keep-alive-123")
        self.assertTrue(result.success)
        self.assertEqual(result.error_message, "")

        MockRefreshSessionIdleTimeRequest.assert_called_once_with(
            authorization="Bearer test_api_key",
            session_id="test_session_id"
        )
        self.agb.client.refresh_session_idle_time.assert_called_once_with(mock_request)

    @patch("agb.session.RefreshSessionIdleTimeRequest")
    def test_keep_alive_api_failure(self, MockRefreshSessionIdleTimeRequest):
        """Test keep_alive when API returns failure"""
        from agb.model.response import OperationResult
        ...
        # Mock the response methods to return failure
        mock_response.is_successful.return_value = False
        mock_response.request_id = "request-keep-alive-456"
        mock_response.get_error_message.return_value = "Session not found"

        result = self.session.keep_alive()

        self.assertIsInstance(result, OperationResult)
        self.assertEqual(result.request_id, "request-keep-alive-456")
        self.assertFalse(result.success)
    ...
    @patch("agb.session.RefreshSessionIdleTimeRequest")
    def test_keep_alive_exception(self, MockRefreshSessionIdleTimeRequest):
        """Test keep_alive when exception occurs"""
        from agb.model.response import OperationResult

        mock_request = MagicMock()
        MockRefreshSessionIdleTimeRequest.return_value = mock_request
        self.agb.client.refresh_session_idle_time.side_effect = Exception("Network error")
        ...
        self.assertFalse(result.success)
        self.assertIn("Failed to refresh session idle time", result.error_message)
