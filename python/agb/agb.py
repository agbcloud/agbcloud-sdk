...
class AGB:
    ...
    def __init__(self, api_key: str = "", cfg: Optional[Config] = None):
        ...
        # Log request base URL so user can tell prod vs pre-release
        base_url = (
            self.endpoint
        ...
        logger.info(
            f"AGB client initialized: request base URL (domain) = {base_url}",
        )
        ...
        if params.browser_context:
            syncs.append(
                ContextSync(
                    ...
                    policy=create_browser_sync_policy(
                        params.browser_context.auto_upload
                    ),
                )
            )
        ...
        for context_sync in context_syncs:
            if context_sync.policy:
                policy_json = json.dumps(
                    context_sync.policy.to_dict(), ensure_ascii=False
                )
    ...
    def create(self, params: Optional[CreateSessionParams] = None) -> SessionResult:
        ...
        try:
            ...
            if not params.image_id or (
                isinstance(params.image_id, str) and not params.image_id.strip()
            ):
            ...
            # Add idle release timeout if provided (in seconds)
            if params.idle_release_timeout is not None:
            ...
            persistence_data_list = self._create_persistence_data_list(
                self._normalize_context_syncs(params)
            )
            ...
            if response.data and response.data.success is False:
                error_msg = response.data.err_msg or "Unknown error"
                log_operation_error("AGB.create", error_msg)
                return SessionResult(
            ...
            if not session_id:
                error_msg = (
                    response.get_error_message() or "Session ID not found in response"
                )
                log_operation_error("AGB.create", error_msg)
                return SessionResult(
                    request_id=request_id,
    ...
    def list(
        ...
        try:
            ...
            logger.info(
                f"Making list_sessions API call - Labels={labels_json}, MaxResults={limit}"
            )
    ...
    def delete(self, session: Session, sync_context: bool = False) -> DeleteResult:
        ...
        """
        log_operation_start(
            "AGB.delete", f"SessionId={session.session_id}, SyncContext={sync_context}"
        )
        try:
