"""
Moltbot on AGB — Reference Example

This script demonstrates an end-to-end workflow for provisioning a cloud session
with Moltbot preinstalled, configuring the Discord channel and optional model
...
3. Enable and configure Discord when DISCORD_BOT_TOKEN is set.
4. Restart the Moltbot gateway and launch the in-session browser to the console URL.
5. Wait for user confirmation (Ctrl+Q), then delete the session.
...
Discord setup
-------------
If you use the Discord channel, follow the official Discord (Bot API) guide first:
create a bot, enable Message Content Intent (and Server Members Intent if needed),
then set DISCORD_BOT_TOKEN. See: https://docs.openclaw.ai/channels/discord
...
Optional (channels and models):
  ...
  DISCORD_BOT_TOKEN       Enables and configures the Discord plugin when set.
...
-----
  ...
  python openclaw_config_example.py
...
def wait_for_ctrl_q() -> None:
    """
    Block until the user presses Ctrl+Q (or Enter as fallback).

    Prompts the user to press Ctrl+Q in the terminal to continue; then the
    ...
    print("When finished, press Ctrl+Q in this terminal to release the session.")
    try:
...
def main() -> None:
    ...
    discord_bot_token = _get_optional_env("DISCORD_BOT_TOKEN")
    ...
    try:
        ...
        if discord_bot_token is not None:
            enable_parts.append("moltbot plugins enable discord")
            config_set_parts.append(
                f'moltbot config set channels.discord.token "{discord_bot_token}"'
            )
            config_set_parts.append(
                f'moltbot config set channels.discord.groupPolicy "open"'
            )
        ...
        if config_parts_list:
            ...
            print(
                "\nNo channel credentials set (DISCORD_BOT_TOKEN). "
