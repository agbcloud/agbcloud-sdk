...
try:
    ...
    logger.warning(
        "Playwright not available. Browser interaction tests will be skipped."
    )
...
async def ensure_listing_page(
    ...
    for i in range(max_steps):
        ...
        if i < max_steps - 1:
            logger.info(
                "Extraction failed, attempting to navigate to a listing page..."
            )
            await act(agent, page, common_action)
            await asyncio.sleep(0.6)
...
async def process_site(agent, url: str, out_dir: str = "/tmp") -> None:
    try:
        if not PLAYWRIGHT_AVAILABLE or async_playwright is None:
            logger.warning(
                "    ⚠️ Playwright not available, skipping browser API detection"
            )
